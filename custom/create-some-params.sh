...
echo "Saving ${config_aws_ssm_path}/spring.datasource.url"
aws ssm put-parameter \
    --region "${config_aws_ssm_region}" \
    --name "${config_aws_ssm_path}/spring.datasource.url" \
    --value "jdbc:postgresql://localhost/postgres" \
    --type String

echo "Saving ${config_aws_ssm_path}/spring.datasource.username"
aws ssm put-parameter \
    --region "${config_aws_ssm_region}" \
    --name "${config_aws_ssm_path}/spring.datasource.username" \
    --value "postgres" \
    --type String

echo "Saving ${config_aws_ssm_path}/spring.datasource.password"
aws ssm put-parameter \
    --region "${config_aws_ssm_region}" \
    --name "${config_aws_ssm_path}/spring.datasource.password" \
    --value "v3ry_s3cr3t" \
    --overwrite \
    --type SecureString
